...
export default function PasswordForm({ user }: PasswordFormProps) {
  ...
  const onSubmit = async (formInputData: PasswordFormValuesSchema) => {
    try {
      ...
      const response = await fetch(`/api/users/${params.userId}/password`, {
        method: "PATCH",
        body: JSON.stringify({
      ...
      router.refresh();
      router.push(`/${params.clinicId}/users/${params.userId}/my-settings/`);
    ...
    <>
      <div className="flex items-center justify-between">
        ...
        <Button disabled={isLoading}>
          <Link href={`/${params.clinicId}/users/${params.userId}/my-settings`}>
            Back to My Profile
          </Link>
        </Button>
      </div>
